...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        ...
        points = roll_dice(num_rolls, dice) # 此处不可写dice=six_sided因为这样会重新赋值dice以至于每次运行只会跑第一次抛的数
...
def is_swap(player_score, opponent_score):
    ...
    if abd(player_score) == abd(opponent_score):
        return True
    else:
        return False
    # END PROBLEM 4
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    if who == 0:
        if h > previous_high:
            print(h, 'point(s)! That's the biggest gain yet for Player 0‘)
    elif who == 1:
        if h > previous_high:
            print(h, 'point(s)! That's the biggest gain yet for Player 1’)
