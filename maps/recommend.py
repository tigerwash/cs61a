...
def find_closest(location, centroids):
    ...
    dis_list = []
    for i in centroids:
        dis_list.append(distance(location, i))
...
def group_by_first(pairs):
    ...
    for key, _ in pairs:
        if key not in keys:
            keys.append(key)
    return [[y for x, y in pairs if x == key] for key in keys]   #remember this line, double itertation
...
def group_by_centroid(restaurants, centroids):
    ...
    #1. find the closest centroid for one restaurant in restaurants list, 'i' represents one restaurant
    #2. find the closest centroid for every restaurant
    #3. get the list [centroid, restaurant]
    #4. use group_by_first to find the restaurant groups with common centroid

    #solution 2
    lst = []
    for i in restaurants:
        restaurant_i = restaurant_location(i)
        centroid_i = find_closest(restaurant_i, centroids)
        lst.append([centroid_i, restaurant_i])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            dic[restaurant_name(i)] = user_rating(user, restaurant_name(i))
        else:
            dic[restaurant_name(i)] = predictor(i)
    return dic
