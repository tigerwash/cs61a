...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        ...
        if t.branches[0].max > t.label and t.branches[1].min <= t.label:
            return False
    elif len(t.branches) == 1:
        if t.branches[0].max > t.label and t.branches[0].min <= t.label:
            return False
    for b in t.branches:
        return is_bst(b)
# Q8
...
def in_order_traversal(t):
    ...
    # https://leetcode.com/problems/binary-tree-inorder-traversal/discuss/703937/Python-Recursive-to-Iterative-to-Morris-Traversal
# Linked List Class
class Link:
    """A linked list.
